...
勾选 **启动时最小化到托盘** 后，应用启动时只驻留托盘：设置窗口完全不显示，按键监听
照常运行，双击托盘图标即可打开窗口。与 **登录 Windows 时自动启动** 搭配使用即可实现
开机后台静默自启——自启动项会注册为
`"…\ms-audio-dock-remapper.exe" --minimized`，因此即使配置文件读取失败，静默启动
依然生效。你也可以给自己的快捷方式加上 `--minimized` 参数达到同样效果。
